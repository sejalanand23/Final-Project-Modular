<template>
  ...
  <br>
    ...
    <tbody>
      ...
      <td><button @click="deleteCard(card.card_id)" class="btn btn-outline-dark">Delete</button></td>
      </tr>
    </tbody> 
  </table>
  </div>
...
async created(){
    ...
    return fetch(`http://127.0.0.1:5000/api/quiz/${this.email}/${this.deck_name}`, {
              method: "GET",
              headers: {
  ...
  methods : {
    async deleteCard(card_id){
      try {
            const card_data = fetch(`http://127.0.0.1:5000/api/card/${card_id}`, {
              method: "DELETE",
              headers: {
                     'Content-Type':'application/json;charset=utf-8',
                     'Authentication-Token': `${this.auth_token}`
